...
class BaseLock extends Lock {
    ...
    constructor(storedContent) {
        ...
        this.commitId = match[3];
        if (match[2].startsWith("a ") || match[2].startsWith("d ") || match[2].startsWith("c ")) {
            this.diff = match[2];
        }
        else {
            this.files = BaseLock.parseFilesString(match[2]);
            this.filesString = match[2];
        }
    ...
    applyDiff(cache) {
        ...
        if (cache[parentName] === undefined) {
            ...
            cache[parentName] = {
                parentName: "gitlock-000-" + parentLock.parentHashes[0],
                filesString: parentLock.filesString
            };
        }
...
let add = (name, content, commitId, time) => {
    $git.run(
        [
