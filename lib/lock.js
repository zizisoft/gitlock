...
class BaseLock extends Lock {
    ...
    fillContent() {
        ...
        if ($cache.hashToLock[parentName] === undefined) {
            let parentLock = Lock.fromName(parentName);
            if (parentLock.filesString === undefined) {
                parentLock.fillContent();
            }
            $cache.hashToLock[parentName] = parentLock;
        }
        this.filesString = $diff.applyDiff($cache.hashToLock[parentName].filesString, this.diff);
