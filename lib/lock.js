...
class Lock {
    ...
    isMini() {
        return this.storedContent === undefined && this.hash !== undefined;
    }

    toMini() {
        return Lock.makeMini(this.name);
    }

    toNormal(usesPdCache) {
        return Lock.fromMini(this, usesPdCache);
    }
    ...
    static fromName(name, usesPdCache) {
        ...
        if ($cache.hashToLock[hash] !== undefined) {
            lock = $cache.hashToLock[hash];
        }
        else if (usesPdCache && $cache.inProgramData(hash)) {
            lock = this.parse($cache.getPdLockContent(hash), hash);
            lock.commitId = getCommitId(name);
            lock.name = name;
        }
        else {
            ...
            lock.commitId = lines[0].match(/^object (.*)$/)[1];
            lock.name = name;
        }
        return lock;
    }

    static makeMini(lockName) {
        let lock = new this(undefined, getHash(lockName));
        lock.name = lockName;
        return lock;
    }

    static fromMini(miniLock, usesPdCache) {
        return this.fromName(miniLock.name, usesPdCache);
    }
}
...
class BaseLock extends Lock {
    ...
    verify(commitId) {
        ...
        assert(testFiles.length === this.files.length);
...
class TimestampLock extends Lock {
    ...
    verify(parent) {
        super.verify();
        if (config.rootCa === undefined) {
            throw new Error("no-root-ca");
        }
...
let getTime = name => {
    let time = $git.line(["for-each-ref", "--format=%(taggerdate:rfc)", "refs/tags/" + $v.lockName(name)]);
    return new Date(time).toISOString();
};

let getCommitId = name => {
    return $git.line(["for-each-ref", "--format=%(object)", "refs/tags/" + $v.lockName(name)]);
};
...
let headHasLocks = () => getNamesFromCommit($git.getLastCommitId()).length > 0;
...
let add = (name, storedContent, commitId, time) => {
    ...
    // Must pass all environment variables to git, otherwise on Ubuntu
    // it will say "unable to auto-detect email address".
...
let getAllLocks = () => {
    let lines = $git.lines([
        "for-each-ref",
        "--format=%(object) %(refname:short) %(taggerdate:rfc)",
        "refs/tags/gitlock-*"
    ]); // use the default "refname" sort order
    return lines.map(line => {
        let parts = line.split(" ");
        let lock = Lock.makeMini(parts[1]);
        lock.commitId = parts[0];
        lock.taggerTime = new Date(parts.slice(2).join(" ")).toISOString();
        return lock;
    });
};
...
exports.headHasLocks = headHasLocks;
