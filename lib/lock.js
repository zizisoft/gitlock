...
class BaseLock extends Lock {
    ...
    fillContent() {
        ...
        if ($cache.hashToLock[parentHash] === undefined) {
            let parentLock = Lock.fromName(parentName);
            if (parentLock.filesString === undefined) {
                parentLock.fillContent();
            }
            $cache.hashToLock[parentHash] = parentLock;
        }
        this.filesString = $diff.applyDiff($cache.hashToLock[parentHash].filesString, this.diff);
