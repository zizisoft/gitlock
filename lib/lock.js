...
class Lock {
    ...
    // Cache doesn't hold stored content. The default value is true.
    // But the problem is that when using cache, if a lock is cached then `storedContent`
    // will be the same as `content`, which in some cases we want to avoid.
    // In those cases you can set it to false.
    static fromName(name, usesCache) {
        if (usesCache === undefined) {
            usesCache = true;
        }
        let hash = getHash(name);
        let path = $path.join($programData.path, hash);
        if (usesCache && $fs.existsSync(path)) {
            return this.parse($fs.readFileSync(path, {encoding: "utf8"}), hash);
        }
        else {
...
class BaseLock extends Lock {
    ...
    // Note: This is the cache in memory. Not to be confused with the program data cache.
    fillContent(cache) {
    ...
    // If `base` already has `filesString` field, then `cache` has no effect.
    fillDiff(base, cache) {
        if (base.filesString === undefined) {
            base.fillContent(cache);
        }
        this.diff = $diff.computeDiff(base.filesString, this.filesString);
        this.storedContent = this.contentBefore + this.diff + this.contentAfter;
    }
...
let getAllLocks = () => {
    ...
    return lines.map(line => {
        ...
        return {name: parts[1], hash: $lock.getHash(parts[1]), commitId: parts[0], taggerTime: parts[2]};
    });
};
