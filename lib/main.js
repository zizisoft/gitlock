...
let addLocks = () => {
    ...
    let processCommit = commitId => {
        ...
        if (commit.isNew) {
            ...
            if (firstParentId !== undefined) {
                ...
                if (!(parentCommit.locks[0] instanceof $lock.BaseLock)) {
                    parentCommit.locks[0] = $lock.Lock.fromMini(parentCommit.locks[0]);
                }
                lock.fillDiff(parentCommit.locks[0]);
            }
        ...
        commit.parentIDs.forEach(parentId => {
            ...
            if (
                commit.locks[0] instanceof $lock.BaseLock &&
                commit.childIDs.every(m => commits[m].traversed)
            ) {
                delete $cache.hashToLock[commit.locks[0].hash];
                commit.locks[0] = commit.locks[0].toMini();
            }
        });
    ...
    let lastLock = commits[lastCommitId].locks[0];
    if (lastLock instanceof $lock.BaseLock) {
        $cache.addPdBaseLock({
            hash: lastLock.hash,
            repo: firstCommitId,
            commit: lastCommitId,
    ...
    commitsArray.forEach((commit, index) => {
        commit.locks.forEach((lock, index) => {
            ...
            if (
                ...
                callback(lock, commit, commits, allCommitsInfo.commits);
            }
        });
...
let verify = args => {
    let errorLockNames = [];
    scanLocks(args, (lock, commit, commits, allCommits) => {
        try {
            if (lock instanceof $lock.BaseLock) {
                ...
                let parentMiniLock = commit.locks[parseInt($lock.getLabel(lock.name)) - 1];
...
try {
    if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
        ...
        let list = [];
        scanLocks(args.slice(1, args.length - 1), (lock, commit) => {
            let label = $lock.getLabel(lock.name);
