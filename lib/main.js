...
let getTagMessageAsync = tag => {
    return new Promise((resolve, reject) => {
        $cp.exec("git show-ref --tags -s " + tag, {encoding: "utf8"}, (err, stdout) => {
            if (err !== null) {
                reject(err);
                return;
            }
            let tagId = $util.parseLine(stdout);
            $cp.exec("git cat-file tag " + tagId, {encoding: "utf8"}, (err, stdout) => {
                if (err !== null) {
                    reject(err);
                    return;
                }
...
if (args.length === 0) {
            ...
            if (commits[commit].isNew) {
                ...
                let traverseDirectory = (id, pathPrefix) => {
                    ...
                    if (parsedTree === undefined) {
                        parsedTree = parseTree($cp.execSync("git cat-file tree " + id));
    ...
    if (type === "--all") {
        ...
        $cp.execSync("git tag -d " + tags.join(" "));
    }
}
