...
let addLocks = () => {
    ...
    if (commits[lastCommitId].lockContent !== undefined) {
        ...
        let MAX_CACHE_LENGTH = 5;
        for (let i = 0; i < config.baseLockCache.length - MAX_CACHE_LENGTH + 1; i++) {
            let hash = config.baseLockCache.shift();
            let cachePath = $path.join($programData.path, hash);
            $fs.unlinkSync(cachePath);
        }

        config.baseLockCache.push(hash);
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    lockNames.forEach(name => {
        ...
        console.log("");
        console.log($lock.getStoredContent(name));
    });
}
else if (args[0] === "list") {
