    ...
    scanLocks(args.slice(1), (lock, lockName, commit, commits) => {
        try {
            if (lock instanceof $lock.BaseLock) {
                ...
                let parentCommitIDs = commit.parentIDs;

                // If not starting from the first commit, the starting commit's parents
                // will be hidden, so we should dig them out.
                if (parentCommitIDs.length === 0) {
                    parentCommitIDs = $git.getParentCommitIDs(commit.id);
                }
