...
let scanLocks = (args, callback) => {
    ...
    if (args[0] === "--all") {
        ...
        if (args.length === 0 && !$lock.headHasLocks()) {
    ...
    commitsArray.forEach((commit, index) => {
        commit.lockNames.forEach(lockName => {
            ...
            if (commitId === null || commit.id === commitId) {
                // The last argument denotes whether it's the specified starting commit.
                // Currently "specified starting commit" just means "specified commit".
                // Later we may support range.
                callback(lock, lockName, commit, commits, commitId !== null && commit.id === commitId);
            }
        });
...
try {
    ...
    console.log("Warning: OpenSSL not found, or OpenSSL version lower than 1.0");
}
...
try {
    if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
        ...
        let errorLockNames = [];
        scanLocks(args.slice(1), (lock, lockName, commit, commits, isSpecifiedStarting) => {
            try {
                if (lock instanceof $lock.BaseLock) {
                    lock.verify(commit.id);

                    // If specifying a commit, its parents will be hidden. We should dig them out.
                    let parentCommitIDs =
                        isSpecifiedStarting ? $git.getParentCommitIDs(commit.id) : commit.parentIDs;
