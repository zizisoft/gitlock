...
let lock = () => {
    ...
    let loadAllLocks = () => {
        ...
        lines.forEach(line => {
            ...
            let lock = parts[1];
            let time = parts[2];
            commits[commit].lockHashes.push(lock);
            commits[commit].time = time;
    ...
    let traverse = commit => {
        if (commits[commit].isNew) {
            ...
            commits[commit].lockHashes.push($lock.computeHash(tagMessage));
            ...
            $git.run(
                [
                    ...
                    "gitlock-000-" + $v.hash(commits[commit].lockHashes[0]),
                    $v.id(commit)
                ],
                {
