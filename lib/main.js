    ...
    let loadAllTags = () => {
        return new Promise((resolve, reject) => {
            ...
            if (tags.length === 0) {
                ...
                tags.forEach(tag => {
                    let doTask = () => {
                        $lock.getTagMessageAsync(tag).then(message => {
                            let parsedMessage = $lock.parseTagMessage(message);
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    let firstTagTime = $lock.getTagTime(tags[0]);
    let request = $cp.execFileSync(
        config.openssl,
        ["ts", "-query", "-cert", "-sha256"],
        {input: $lock.getTagMessage(tag)}
    );
    ...
    let clientRequest = $http.request({
        ...
        serverResponse.on("end", () => {
            let newMessage =
                "timestamps\n" +
                "\n" +
                "parent " + $lock.getTagHash(tag) + "\n" +
                "\n" +
                $lock.generateBase64(data) + "\n" +
                "\n" +
                $lock.generateNonce();
            let newTag = "gitlock-" + $lock.generateTagLabel(tag) + "-" + $lock.computeHash(newMessage);
            $git.run(
                [
                    "tag", "-a", "-F", "-", "--cleanup=verbatim",
                    $v.tagName(newTag),
    ...
    tags.forEach((tag, index) => {
        let message = $lock.getTagMessage(tag);
        let hash = $lock.getTagHash(tag);
...
`;
    ...
    let tags = $git.lines([
        "tag", "-l", "--points-at", $v.idOrHashOrTagName(args[1]), "--sort=refname", "gitlock-*"
    ]);
    tags.forEach(tag => {
        console.log(tag);
        console.log("=".repeat(tag.length));
        console.log("");
        console.log($lock.getTagMessage(tag));
    });
}
else if (args[0] === "list") {
    ...
    if (type === "--all") {
        ...
        $git.run(["tag", "-d"].concat(tags.map(m => $v.tagName(m))));
    }
}
