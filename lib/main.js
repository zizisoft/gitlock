...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    // "--points-at" also applies to tag names. So it also works if the arg is a lock name.
    // TODO: Currently do not support hash name
    let usesFullContent = args[1] === "--full-content";
    let itemToShow = usesFullContent ? args[2] : args[1];
    let lockNames = $git.lines([
        "tag", "-l", "--points-at", $v.idOrHashOrLockName(itemToShow), "--sort=refname", "gitlock-*"
    ]);
    lockNames.forEach(name => {
        ...
        if (usesFullContent) { // currently this is only for testing purposes
            console.log($lock.Lock.fromName(name).getContent());
        }
        else {
