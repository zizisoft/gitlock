...
let addLocks = () => {
    ...
    let processCommit = commitId => {
        ...
        if (commit.isNew) {
            ...
            $cache.hashToLock[lock.hash] = lock;
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    if (args[1] === "--all") {
        commits = loadCommits().commitsArray;
    }
    else {
        commitId = args.length === 3 ? args[1] : $git.getLastCommitId();
        ...
        if ($cache.inProgramData(lockHashes[0])) {
            commits = loadCommits(commitId).commitsArray;
        }
        else {
            commits = loadCommits().commitsArray;
        }
    ...
    commits.forEach((commit, index) => {
        ...
        let now = new Date().toISOString();
        if ($util.subtractTime(now, progressTime) >= 1) {
            console.log(`${index + 1} of ${commits.length} commits processed.`);
            progressTime = now;
        }
    });
