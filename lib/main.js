...
let getTagMessage = tag => {
    let tagId = $util.parseLine($cp.execSync("git show-ref --tags -s " + vTagName(tag), {encoding: "utf8"}));
    let lines = $util.parseLines($cp.execSync("git cat-file tag " + vId(tagId), {encoding: "utf8"}));
...
let getTagMessageAsync = tag => {
    return new Promise((resolve, reject) => {
        $cp.exec("git show-ref --tags -s " + vTagName(tag), {encoding: "utf8"}, (err, stdout) => {
            if (err !== null) {
                reject(err);
                return;
            }
            let tagId = $util.parseLine(stdout);
            $cp.exec("git cat-file tag " + vId(tagId), {encoding: "utf8"}, (err, stdout) => {
                if (err !== null) {
                    reject(err);
                    return;
                }
...
let lock = () => {
    ...
    let loadAllTags = () => {
        return new Promise((resolve, reject) => {
            let tags = $util.parseLines(
                $cp.execSync(`git tag -l --sort=refname "gitlock-*"`, {encoding: "utf8"})
            );
            if (tags.length === 0) {
                resolve();
            }
    ...
    let traverse = commit => {
        if (commits[commit].isNew) {
            let rawCommitObject = $cp.execSync("git cat-file commit " + vId(commit), {encoding: "utf8"});
            ...
            let traverseDirectory = (id, pathPrefix) => {
                ...
                if (parsedTree === undefined) {
                    parsedTree = parseTree($cp.execSync("git cat-file tree " + vId(id)));
            ...
            files.forEach(file => {
                ...
                if (hash === undefined) {
                    if (file.mode.startsWith("040")) {
                        ...
                        let bytes = $cp.execSync("git cat-file blob " + vId(file.id));
            ...
            $cp.execSync(
                ...
                "git tag -a -F - --cleanup=verbatim gitlock-000-" +
                vHash(commits[commit].locks[0]) +
                " " +
                vId(commit),
                {
                    encoding: "utf8",
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    let status = $util.parseLines($cp.execSync("git status --porcelain", {encoding: "utf8"}));
    if (status.some(m => m[0] !== " " && m[0] !== "?" && m[0] !== "!")) {
        if (args.length === 0) {
            $cp.execSync("git commit --allow-empty-message");
        }
        else if (args[0] === "-m") {
    ...
    let tags = $util.parseLines(
        ...
        $cp.execSync(
            `git tag -l --points-at ${vIdOrHashOrTagName(args[1])} --sort=refname "gitlock-*"`,
            {encoding: "utf8"}
        )
    );
    tags.forEach(tag => {
    ...
    if (type === "--all") {
        let tags = $util.parseLines(
            $cp.execSync("git tag -l \"gitlock-*\"", {encoding: "utf8"})
        );
        $cp.execSync("git tag -d " + tags.map(m => vTagName(m)).join(" "));
    }
}
