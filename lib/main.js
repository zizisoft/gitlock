...
// The `traversed` field of every commit can't be `true` before calling this function.
// It will be changed to `true` before (not after) calling each `callback`.
let traverseCommits = (commits, firstCommitId, callback) => {
    let stack = [];
    let parentId = null;
    let index = null;
    let commitId = firstCommitId;
    commits[commitId].traversed = true;
    callback(firstCommitId);
    parentId = firstCommitId;
    index = 0;
    commitId = commits[parentId].childIDs[index];
    while (commitId !== undefined) {
        if (commits[parentId].childIDs.length > index + 1) {
            stack.push({parentId: parentId, index: index + 1});
        }

        if (commits[commitId].parentIDs.every(m => commits[m].traversed)) {
            commits[commitId].traversed = true;
            callback(commitId);
            parentId = commitId;
            index = 0;
...
let addLocks = () => {
    ...
    let loadAllLocks = () => {
        ...
        lines.forEach(line => {
            ...
            let lockHash = $lock.getHash(parts[1]);
            let time = parts[2];
            ...
            if (commits[commitId] !== undefined) {
                commits[commitId].lockHashes.push(lockHash);
    ...
    traverseCommits(commits, firstCommitId, traverseCallback);
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    if (type === "--all") {
        ...
        if (inCommit) {
            ...
            let commits = {};
            $git.lines([
                "rev-list", "--ancestry-path", "--children", "--boundary", "--reverse",
            ...
            $git.lines([
                "rev-list", "--ancestry-path", "--parents", "--boundary",
                $v.id(startingCommitId) + "..HEAD"
            ]).forEach(m => {
                if (m.startsWith("-")) {
                    if (m.startsWith("-" + startingCommitId)) {
            ...
            // Although "--ancestry-path" can reduce lines, it can't suppress the
            // already-filtered commits shown as parents and children on the right.
            // We should remove them.
            Object.keys(commits).forEach(commitId => {
                let commit = commits[commitId];
                commit.childIDs = commit.childIDs.filter(m => commits[m] !== undefined);
                commit.parentIDs = commit.parentIDs.filter(m => commits[m] !== undefined);
            });
            ...
            let loadAllLocks = () => {
                ...
                lines.forEach(line => {
                    ...
                    let lockName = parts[1];
                    if (commits[commitId] !== undefined) {
                        commits[commitId].lockNames.push(lockName);
                    }
                });
            };
            ...
            let traverse = commitId => {
                ...
                commit.lockNames.forEach(name => lockNames.push(name));
