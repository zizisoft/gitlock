...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    if (type === "--all") {
        ...
        if (inCommit) {
            ...
            let commits = {};
            [startingCommitId].concat($git.lines([
                "rev-list", "--ancestry-path", "--children", "--reverse",
            ...
            $git.lines([
                "rev-list", "--ancestry-path", "--parents", $v.id(startingCommitId) + "..HEAD"
            ]).concat([startingCommitId]).forEach(m => {
                let parts = m.split(" ");
                commits[parts[0]].parentIDs = parts.slice(1);
            });
