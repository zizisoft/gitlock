...
let lock = () => {
    ...
    let traverse = commitId => {
        ...
        if (commit.isNew) {
            let rawCommitObject = $git.run(["cat-file", "commit", $v.id(commitId)]);
            ...
            if (commit.parents.length > 0) {
                commit.parents.forEach(parent => {
                    tagMessagePart1 += "parent " + commits[parent].lockHashes[0] + "\n";
                });
                tagMessagePart1 += "\n";
            }
            let tagMessagePart2 = "";
            ...
            commit.lockHashes.push($lock.computeHash(tagMessage));
            commit.filesString = tagMessagePart2;
...
if (args.length === 0 || args[0] === "-m" || args[0] === "commit") {
    ...
    tags.forEach((tag, index) => {
        ...
        list.push(commitId + " " + pad(index, 3) + " " + hash + "\n");
    });
